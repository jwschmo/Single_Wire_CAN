/**
 * Edited by John W. Schmotzer Jan 4 2011
 * Notes: Added funcationality to message_rx and message_tx funcations specifically:
 * message_rx now updates a pointer for the ID for the message allowing user to see received message ID
 * message_tx now sends user specified data that is determined when the function is called.
 *
...
char CanbusClass::message_rx(unsigned char *buffer,uint16_t *CAN_addr) {
		tCAN message;
...
char CanbusClass::message_tx(unsigned char *buffer, uint16_t *CAN_addr) {
	tCAN message;

	message.id = *CAN_addr;    /*JWS*/
	...
	message.data[7] = buffer[7];	/*JWS*/

	/* Editing code to allow for transmission accross any address. */
	// einige Testwerte
	/*message.id = 0x7DF;
	message.header.rtr = 0;
	message.header.length = 8;
